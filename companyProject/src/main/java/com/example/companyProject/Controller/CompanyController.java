...
public class CompanyController implements CompanyControllerInterface {
    ...
    @GetMapping
    public Page<CompanyProjection> getAllCompanies(@RequestParam(required = false) String name , @RequestParam(required = false) CompanyStatus status, Pageable pageable){
